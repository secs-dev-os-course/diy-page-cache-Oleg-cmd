...
TEST(StorageTest, ReadWrite)
{
  ...
  size_t data_size = strlen(test_data);
    size_t aligned_size = ((data_size + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE; 
...
TEST(StorageTest, ReadWriteOffset)
{
  ...
  ASSERT_EQ(storage.open(O_RDWR | O_CREAT | O_DIRECT), 0);
    ...
    size_t data_size1 = ((strlen("Test data 1") + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE; 
    ...
    size_t data_size2 = ((strlen("Test data 2") + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
