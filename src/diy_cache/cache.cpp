...
Cache::Cache(PageSize pageSize, CacheSize cacheSize)
    : cacheSize(cacheSize), pageSize(pageSize), hits(0), misses(0)
{
                 ...
                 off_t offset) -> ssize_t
{
  ...
  while (count > 0)
  {
    LOG("Cache::read: loop start, current_offset=" << current_offset
                                                   << ", count=" << count);
    std::lock_guard<std::mutex> lock(cacheMutex);
    ...
    size_t offsetInPage = static_cast<size_t>(
        static_cast<size_t>(current_offset) % pageSize.getValue());
    ...
    LOG("Cache::read: loop end, totalBytesRead="
        << totalBytesRead << ", current_offset=" << current_offset
        << ", count=" << count);
  }
...
int Cache::sync(Storage* storage)
{
  ...
  for (auto& page : pageList)
  {
    ...
    {
      size_t aligned_size =
          ((pageSize.getValue() + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
      ...
      ssize_t bytesWritten =
          storage->write(page.getOffset(), aligned_buf, aligned_size);
      free(aligned_buf);
...
Page* Cache::getPage(Storage* storage, off_t offset)
{
  ...
  ssize_t bytes_read =
      storage->read(offset, page->getData(), page->getPageSize());
  if (bytes_read < 0)
  {
    pageList.pop_back();
    return nullptr;
  ...
  for (auto it = pageList.begin(); it != pageList.end(); ++it)
  {
    if (!it->isAccessed())
    {
      if (it->isDirty())
      {
        ...
        if (!Storage::checkAlignment(it->getData(), it->getOffset(),
                                     aligned_size))
        {
          void* aligned_buf = nullptr;
          if (posix_memalign(&aligned_buf, BLOCK_SIZE, aligned_size) == 0)
