...
ssize_t lab2_read(int fd, void* buf, size_t count) {
    ...
    size_t total_read = 0;
      LOG("lab2_read: start, fd=" << fd << ", offset=" << offset << ", count=" << count);
     while (total_read < count) {
        size_t current_offset = offset + total_read;
        size_t bytes_to_read = std::min(page_size, count - total_read); 
        LOG("lab2_read: loop start, current_offset=" << current_offset << ", bytes_to_read=" << bytes_to_read << ", total_read=" << total_read);
        ssize_t read_result = g_cache->read(storage, out_buf + total_read, bytes_to_read, current_offset);
         if (read_result < 0) {
           LOG("lab2_read: g_cache->read returned error or 0, read_result=" << read_result);
            return read_result;
         }
        if(read_result == 0) {
         ...
         LOG("lab2_read: loop end, read_result=" << read_result << ", total_read=" << total_read);
    }
...
ssize_t lab2_write(int fd, const void *buf, size_t count)
{
    ...
    while (total_written < count) {
        ...
        size_t bytes_to_write = std::min(page_size - offset_in_page, count - total_written);

        if (offset_in_page != 0 || bytes_to_write != page_size) {
            ssize_t read_result = g_cache->read(storage, aligned_buf, page_size, page_aligned_offset);
        ...
        memcpy(aligned_buf + offset_in_page, in_buf + total_written, bytes_to_write);

        ssize_t write_result = g_cache->write(storage, aligned_buf, page_size, page_aligned_offset);
        if (write_result < 0) {
            free(aligned_buf);
            return write_result;
        }
