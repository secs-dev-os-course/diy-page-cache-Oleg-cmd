...
int Storage::open(int flags, mode_t mode)
{
  ...
  if (fd == -1)
  {
    if (errno == EINVAL)
    {
      ...
      if (fd == -1)
      {
        std::cerr << "Error opening file without O_DIRECT: " << strerror(errno)
                  << std::endl;
        return -errno;
      }
    }
  ...
  if ((flags & O_CREAT) && fileSize == 0)
  {
    ...
    {
      std::cerr << "Warning: fallocate failed: " << strerror(errno)
                << std::endl;
    }
  }
...
ssize_t Storage::read(off_t offset, void* buf, size_t count)
{
  if ((reinterpret_cast<uintptr_t>(buf) & (BLOCK_SIZE - 1)) != 0 ||
      (offset & (BLOCK_SIZE - 1)) != 0 || (count & (BLOCK_SIZE - 1)) != 0)
  {
...
off_t Storage::getFileSize() const { return fileSize; }